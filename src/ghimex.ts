...
function isMatchAllExpression(
  d: IssueLike,
  exps: Expression[] | AllExpression<Expression>,
): boolean {
...
function isMatchAnyExpression(
  d: IssueLike,
  exp: AnyExpression<Expression>,
): boolean {
...
export function isMatch(d: IssueLike, e: Expression): boolean {
  ...
  if ('matchLabel' in e) {
    return (
      d.labels().find((o) => new RegExp(e.matchLabel).test(o)) !== undefined
    );
  }
  if ('any' in e) {
